...
test("send", function(t) {
  ...
  var toSpend = 0.01;
  var txFee = 0.0005;
  bitcoin.getBalance(conn_id, [fromAcct], function(err, origFromBalance) {
    t.error(err, "getBalance() should not error");
    bitcoin.getNewAddress(conn_id, [toAcct], function(err, toAddr) {
      t.error(err, "getNewAddress() should not error");
      ...
      bitcoin.sendMany(conn_id, [fromAcct, many], function (err, txid) {
        ...
        bitcoin.getTransaction(conn_id, [txid], function(err, tx) {
          ...
          */
          bitcoin.getBalance(conn_id, [fromAcct, 0], function(err, newFromBalance) {
            t.error(err, "getBalance() should not error");
            bitcoin.getBalance(conn_id, [toAcct, 0], function(err, newToBalance) {
              t.error(err, "getBalance() should not error");
